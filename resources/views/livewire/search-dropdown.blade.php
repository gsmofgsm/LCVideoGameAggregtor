<div class="relative">
    <input wire:model.debounce.300ms="search"
        type="search" class="bg-gray-800 text-sm rounded-full
                    focus:outline-none focus:shadow-outline w-64 pl-8 px-3 py-1" placeholder="Search...">
    ...
    <div class="absolute z-50 bg-gray-800 text-xs rounded w-64 mt-2">
        ...
        <ul>
            ...
            <li class="border-b border-gray-700">
                <a href="{{ route('games.show', $game['slug']) }}" class="block hover:bg-gray-700 flex items-center transition ease-in-out px-3 py-3">
                    @if($game['cover'])
                    <img src="{{ Str::replaceFirst('thumb', 'cover_small', $game['cover']['url']) }}"
                         class="w-10" alt="cover">
                    @else
                        <img src="https://via.placeholder.com/264x352" alt="game cover" class="w-18">
                    @endif
